...
void main() {
  group('JsonParser', () {
    test('parse() should parse a single JSON object in isolate', () async {
      final result = await JsonParser.parse<int>(<String, dynamic>{
        'value': 10,
      }, _intParser);
    ...
    test('parse() should rethrow parser exception', () async {
      expect(
        () => JsonParser.parse<int>(<String, dynamic>{
          'value': 10,
          'throw': true,
        }, _intParserWithError),
    ...
    test(
      ...
      () async {
        final result = await JsonParser.parseList<int>(<JSON>[

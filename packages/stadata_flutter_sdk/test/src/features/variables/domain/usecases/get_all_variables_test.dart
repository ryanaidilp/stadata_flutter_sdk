...
void main() {
  ...
  setUpAll(() {
    ...
    final response = ApiResponseModel<List<VariableModel>?>.fromJson(json, (
      json,
    ) {
  ...
  group('GetAllVariables', () {
    test('should return list of variables if call success', () async {
      // arrange
      when(
      ...
      // act
      final result = await usecase(const GetAllVariablesParam(domain: domain));

      // assert
      expect(
        result,
        equals(Result.success<Failure, ApiResponse<List<Variable>>>(data)),
      );

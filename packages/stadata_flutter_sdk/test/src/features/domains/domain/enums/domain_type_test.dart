...
void main() {
  group('DomainType', () {
    test(
      ...
      () => expect(DomainType.all.value, 'all'),
    );

    test(
      'DomainType.regency should return kab',
      () => expect(DomainType.regency.value, 'kab'),
    );

    test(
      'DomainType.province should return prov',
      () => expect(DomainType.province.value, 'prov'),
    );

    test(
      'DomainType.regencyByProvince should return kabbyprov',
      () => expect(DomainType.regencyByProvince.value, 'kabbyprov'),
    );
  });
}

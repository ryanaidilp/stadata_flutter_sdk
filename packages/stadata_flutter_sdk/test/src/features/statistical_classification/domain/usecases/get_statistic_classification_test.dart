...
void main() {
  ...
  late GetStatisticClassification usecase;
  ...
  setUpAll(() {
    ...
    usecase = GetStatisticClassification();
  ...
  group('GetStatisticClassification', () {
    test(
      'should return list of statistic classification if call success',
      () async {
        // arrange
        when(
          () => mockRepository.get(type: KBLIType.y2020),
        ).thenAnswer((_) async => Result.success(statisticClassifications));

        // act
        final result = await usecase(
          const GetStatisticClassificationParam(type: KBLIType.y2020),
        );
        ...
        verify(() => mockRepository.get(type: KBLIType.y2020)).called(1);
      },
    );

    test('should return failure if exception is thrown', () async {
      // arrange
      when(() => mockRepository.get(type: KBLIType.y2020)).thenAnswer(

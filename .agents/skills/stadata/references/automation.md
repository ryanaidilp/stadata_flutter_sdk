...
- **`CHANGELOG.md` has a hard 262144-byte (256KB) limit on pub.dev — check the size before every release.** `publish.yml` fails at the upload step (after the tag is already created and pushed!) with `Message from server: CHANGELOG.md exceeds the maximum content length (262144 bytes)`. Root cause hit on 2026-09-05: the file was a concatenation of 6+ separate top-level `# Change Log`/`# Changelog` sections from different changelog tools/runs across the project's history, several with genuine internal duplication (some 0.x release headers appeared 4-8 times). Fixed by truncating to the clean, non-duplicated top section and moving everything else to `CHANGELOG_ARCHIVE.md` (repo root, naturally excluded from the published package since `publish.yml`'s working-directory is `packages/stadata_flutter_sdk/`). **Recovery when this happens again:** the tag already exists pointing at the broken commit — fix `CHANGELOG.md` on `main`, commit, then move the tag (`git tag -d X.Y.Z && git push origin :refs/tags/X.Y.Z && git tag -a X.Y.Z -m "Release X.Y.Z" && git push origin X.Y.Z`) to re-trigger `publish.yml` from the fixed commit. Run `wc -c CHANGELOG.md` before pushing any `release/**` branch — the file had ~106KB of headroom immediately after the 2026-09-05 truncation, but it will fill up again since every release still appends a full commit-by-commit section.
- **A `CHANGELOG_BACKUP.md` at the repo root is maintained automatically by the changelog action itself** (rewritten on every release, going back to at least 1.0.0) — it mirrors whatever `CHANGELOG.md` looked like before each regeneration and is not something an agent creates or should delete; it's a separate, lower-priority growth concern from the pub.dev-facing `CHANGELOG.md` limit above.

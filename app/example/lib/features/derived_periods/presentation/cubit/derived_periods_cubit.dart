...
@injectable
class DerivedPeriodsCubit extends BaseCubit<BaseState> {
  DerivedPeriodsCubit(this._stadataFlutter) : super(const InitialState());
  ...
  Future<void> loadData({int? page}) async {
    ...
    try {
      final result = await _stadataFlutter.list.derivedPeriods(
        domain: _domain!,
      ...
      _derivedPeriods = result.data;
      _totalPages = result.pagination?.pages ?? 1;
      _currentPage = targetPage;

      emit(LoadedState<List<DerivedPeriod>>(_derivedPeriods));

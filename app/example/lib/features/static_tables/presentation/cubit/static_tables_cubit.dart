...
class StaticTablesState extends BaseState {
  const StaticTablesState({required this.baseState, this.stateVersion = 0});
...
@injectable
class StaticTablesCubit extends BaseCubit<BaseState> {
  StaticTablesCubit() : super(const InitialState());
  ...
  // Helper method to extract base state from StaticTablesState or return as is
  BaseState _extractBaseState(BaseState state) {
    if (state is StaticTablesState) {

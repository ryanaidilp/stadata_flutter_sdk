...
@injectable
class StaticTablesResultsCubit extends BaseCubit<BaseState> {
  StaticTablesResultsCubit(this._stadataFlutter) : super(const InitialState());
  ...
  Future<void> loadData({int page = 1}) async {
    ...
    try {
      ...
      const baseUrl = 'https://webapi.bps.go.id/v1/api/list/statictable';
      final queryParams = parameters.entries
          .map((e) => '${e.key}=${e.value}')
          .join('&');
      final fullUrl = '$baseUrl?$queryParams';

      final result = await _stadataFlutter.list.staticTables(
        domain: _domain!,
      ...
      emit(LoadedState<List<StaticTable>>(result.data));

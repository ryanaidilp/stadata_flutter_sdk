...
@injectable
class InfographicsResultsCubit extends BaseCubit<BaseState> {
  InfographicsResultsCubit(this._stadataFlutter) : super(const InitialState());
  ...
  Future<void> loadData() async {
    ...
    try {
      ...
      const baseUrl = 'https://webapi.bps.go.id/v1/api/list/infographic';
      final queryParams = parameters.entries
          .map((e) => '${e.key}=${e.value}')
          .join('&');
      final fullUrl = '$baseUrl?$queryParams';

      final result = await _stadataFlutter.list.infographics(
        domain: _currentDomain,

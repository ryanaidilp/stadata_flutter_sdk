...
@injectable
class DerivedVariablesCubit extends BaseCubit<BaseState> {
  DerivedVariablesCubit(this._stadataFlutter) : super(const InitialState());
  ...
  int? get verticalGroup => _verticalGroup;
  ...
  void setVerticalGroup(int? verticalGroup) {
  ...
  Future<void> loadData({int? page}) async {
    ...
    try {
      final result = await _stadataFlutter.list.derivedVariables(
        domain: _domain!,
        lang: _currentLanguage,
        page: targetPage,
        variableID: _variableID,
        verticalGroup: _verticalGroup,
      );

      _derivedVariables = result.data;
      _totalPages = result.pagination?.pages ?? 1;
      _currentPage = targetPage;

      emit(LoadedState<List<DerivedVariable>>(_derivedVariables));
  ...
  /// Fetch vertical variables with pagination support for SearchableDropdown
  Future<List<VerticalVariable>> fetchVerticalVariables({
    required int page,
    String? searchText,
  }) async {
    if (!DomainValidator.isValid(_domain)) {

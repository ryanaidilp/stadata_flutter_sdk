...
import 'package:stadata_example/features/derived_variables/presentation/widgets/derived_variables_parameters_panel.dart';
import 'package:stadata_example/shared/cubit/base_cubit.dart';

@RoutePage()
class DerivedVariablesParametersPage extends StatelessWidget {
  const DerivedVariablesParametersPage({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => getIt<DerivedVariablesCubit>(),
      child: BlocBuilder<DerivedVariablesCubit, BaseState>(
        builder: (context, state) {
          final cubit = context.read<DerivedVariablesCubit>();

          return Scaffold(
            appBar: AppBar(title: const Text('Derived Variables')),
            body: SingleChildScrollView(
              padding: const EdgeInsets.all(AppSizes.spaceMd),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const DerivedVariablesParametersPanel(),
                                ...
                                unawaited(
                                  context.router.push(
                                    DerivedVariablesResultsRoute(
                                      ...
                                      verticalGroup: cubit.verticalGroup,
                                    ),
                                  ),
                                );
                              }
